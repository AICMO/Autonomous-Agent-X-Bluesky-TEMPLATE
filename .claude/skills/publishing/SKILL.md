...
1. **News hooks** — Significantly higher impressions than average posts
2. **Dollar-amount headlines** — ($285B, $2B, $1T) quantified impact stops scroll
3. **Name-drops** — Well-known figures in your niche boost visibility
4. **Replies to official accounts** — Higher engagement than replies to individuals

**What underperforms:** Generic framework posts without news hook, process posts without news hook, personality without timeliness, stale replies (>6h).

### X Post Length (Premium = up to 25,000 chars)

X Premium unlocks long-form posts. **Write X posts at full length. Bluesky is a separate platform with separate constraints — never let Bluesky's 290-char limit shrink X posts.**
...
> Every post starts from expertise (pillars). News hooks are allowed but only as a doorway into a pillar topic. The news is the hook; the pillar expertise is the value.
...
2. **Pillar connection** — Which pillar does this come from? What's MY angle?
...
3. **Industry analysis through pillar lens** (20%) — Economics, patterns, market shifts that affect our domain
4. **Promotional** (10%) — Repo, live outputs, owner's products — only when the post is genuinely about what we build
...
**Only link the repo when the post is genuinely about building or running agents.** The repo link is proof, not decoration. If the post is about industry trends unrelated to this repo, the link has no business being there.
...
- **Off-pillar news** — Funding rounds, chip specs, acquisitions, or investment analysis that doesn't connect to a pillar. If the post could be written by any generic news account, it's not ours.
- **Forced repo links** — Never graft the repo URL onto an unrelated topic.
- **Don't promote X on X** — Never post "follow me on X" or link to the X profile in X posts. That's circular. Promote the blog, newsletter, repos, or live outputs instead.
- **Politics (HARD BAN)**: Never post about politicians by name (presidents, senators, etc.), legislation, executive orders, Senate/Congress votes, elections, or political parties — even when the topic is AI or tech.
...
- **Queue 13-14 (near limit):** Zero new content. Don't stage. Creating 2 files at 13 pushes queue to 15 → immediate block next session.
- **Queue 11-12 (look-ahead zone):** Max 1 content piece. Creating 2 files at 11 pushes queue to 13 → next session is immediately blocked.
- **Queue <= 10:** Create max 2 content pieces per session. X post is required. Bluesky version optional.
- **Bluesky throttle:** When BS queue >= 10, skip BS file entirely — even if X queue allows content. BS drains slower than X.

**Why the 13-14 zone is blocked (not just >= 15):**
The hard limit is 15. Creating 2 content files per session (the max) at queue=13 pushes to 15 — triggering a block immediately next session. At queue=14, even 1 file hits the limit.
...
**When queue >= 13 AND staged pairs >=20:** 0% content, 0% research, 0% staging. 50% cleanup/memory management, 50% skill work or engagement prep. **Skip PR creation entirely if nothing meaningful to commit.**

**HARD RULE: No empty PRs.** If the session has no new content files, no research files, no skill updates, and no meaningful state changes — do NOT create a PR. "State update only" PRs are waste.
...
**Goal of every CTA:** Drive a measurable action — GitHub stars, blog/newsletter subscribers, leads. Not vanity ("follow me"), but funneling attention into the owner's properties.

**What to promote** (discover from ME.md, don't hardcode):
- Owner's repos and orgs (use discovery skill's OS scan)
- Live agent outputs: blog, newsletter, etc.
- Owner's company when post topic aligns
...
**Rules:**
- CTA must match the post topic. Don't cross-wire.
...
### CTA Discipline
Every post should drive attention toward owner's properties (repos, blog, newsletter, company). Match CTA to post topic. Discover links from ME.md, don't hardcode.
